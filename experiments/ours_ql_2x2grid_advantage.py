...
def multi_objective_reward(ts):
    ...
    current_queue = sum(queues)

    if ts_id not in last_queue:
        last_queue[ts_id] = current_queue
    ...
    switched = int(
        current_phase != last_phase[ts_id]
    )
...
if __name__ == "__main__":
    ...
    for run in range(1, runs + 1):
        ...
        for episode in range(1, episodes + 1):
            ...
            env.save_csv(f"outputs/2x2_our_advantage/ql-2x2grid_run{run}", episode)
